"""Sender implementation for VK messages API."""
...
class VkSender(BaseChannelSender):
    ...
    async def send_text(self, bot_id: int, external_chat_id: str, text: str, attachments=None) -> None:
        ...
        if not channel:
            logger.info("No VK channel configured for bot", extra={"bot_id": bot_id})
            return

        if not channel.is_active:
            logger.info("VK channel is inactive; skipping send", extra={"bot_id": bot_id, "channel_id": channel.id})
            return

        if attachments:
            logger.warning(
                "VK attachments are not supported yet; ignoring",
                extra={
                    "bot_id": bot_id,
        ...
        if not access_token:
            logger.error("VK channel config missing access token", extra={"bot_id": bot_id, "channel_id": channel.id})
            return

        params = {
            "access_token": access_token,
            "v": (channel.config or {}).get("api_version", "5.131"),
        ...
        try:
            async with httpx.AsyncClient(timeout=10) as client:
                response = await client.post(VK_MESSAGES_SEND_URL, data=params)

            if response.status_code != 200:
                logger.error(
                    "VK API responded with HTTP error",
