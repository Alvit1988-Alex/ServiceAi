...
class Bitrix24Service:
    ...
    async def refresh_access_token(self, session: AsyncSession, integration: BitrixIntegration) -> BitrixIntegration:
        ...
        async with httpx.AsyncClient(timeout=20.0) as client:
            response = await client.post(token_url, data=payload)
            if response.status_code >= 400:
                logger.error("Bitrix token refresh failed", extra={"status_code": response.status_code, "bot_id": integration.bot_id})
    ...
    async def call_rest(
        ...
        *,
        ...
        if active_integration.expires_at and active_integration.expires_at <= datetime.utcnow():
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                logger.warning("Bitrix REST call failed", extra={"method": method_name, "attempt": attempt, "bot_id": integration.bot_id})
                if attempt == max_retries:
        ...
        *,
        ...
        if not integration.openline_id:
            raise BitrixIntegrationError(
                "Не задана Open Line (LINE) для Bitrix24. Укажите ID линии в настройках интеграции."
            )
